...
KUBECTL="minikube -p $ROOK_PROFILE_NAME kubectl --"
...
show_info() {
    ...
    echo " "
    echo "    > " alias kubectl=\"minikube -p $ROOK_PROFILE_NAME kubectl --\"
}
...
setup_minikube_env() {
    ...
    minikube start -p $ROOK_PROFILE_NAME --disk-size=40g --extra-disks=3 --driver "$minikube_driver"
    eval "$(minikube docker-env -p $ROOK_PROFILE_NAME)"
}
